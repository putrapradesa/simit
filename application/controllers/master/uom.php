...
class uom extends CI_Controller
{
    public function __construct()
    {
        ...
        $this->load->model('master/m_uom');
    }

    public function index()
    {
        $data['Judul'] = 'Satuan';
        $data['content'] = 'master/uom/home';

        $where = array(
            "IsDelete" => 0
        );

        $datapt = $this->m_uom->cek('uom',$where)->result();
        $data['datauom'] = $datapt;
        $data['modal_tambah_uom'] = show_my_modal('master/uom/modal/add', 'tambah-uom', $data);
        $data['menu'] = 'master';
        $data['submenu'] = 'satuan';
    ...
    public function tampil(){
        ...
        $data['datauom'] = $this->m_uom->cek('uom',$where)->result();
        $this->load->view('master/uom/list', $data);
    }

    public function pt_check() {
        $pt =  $this->input->post('nama_uom');

        $where = array(
            'Nama'      => strtoupper($pt),
            'IsDelete'  => 0 
        );
        $cek = $this->m_uom->cek('uom',$where)->num_rows();
        if($cek > 0 ){
            return TRUE;
        }else{
            return FALSE;
    ...
    public function add(){
        $this->form_validation->set_rules('nama_uom[]', 'Nama Satuan', 'trim|required', 'callback_pt_check');
        ...
        if ($this->form_validation->run() == TRUE) {
                    ...
                    $where = array(
                        'Nama'      => strtoupper($data["nama_uom"]),
                    );
        
                    $cek = $this->m_uom->cek('uom',$where)->num_rows();
                    // $counter = $this->m_pt->getdata('uom')->num_rows();
                    // $NoPt = str_pad($counter + 1, 5, 0, STR_PAD_LEFT);
        
                    if($cek > 0 )
                    {
                        $out['status'] = 'form';
                        $out['msg'] = show_err_msg('Satuan '. $data["nama_uom"] . ' Sudah Ada', '20px');
                        ...
                        array_push($input, array(
                            ...
                            'IsDelete' => 0  // Ambil dan set data alamat sesuai index array dari $index
                        ));
                    }
                    // $index++;
                ...
                if(!empty($input)){
                    $result = $this->m_uom->save_batch($input);
                    if ($result > 0) {
                        $out['status'] = 'form';
    ...
    public function update(){
		...
		$data['datauom'] = $this->m_uom->cek('uom',$where)->row();

		echo show_my_modal('master/uom/modal/update', 'update-uom', $data);
    }


    public function edit()
    {
        $this->form_validation->set_rules('nama_uom', 'Nama Satuan', 'trim|required');
        $data = $this->input->post();
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            $cek = $this->m_uom->cek('uom',$where)->row();
            $data = array(
                'Nama'      => strtoupper($data["nama_uom"]),
			...
			$result = $this->m_uom->update('uom',$data);
    ...
    public function excel(){
        ...
        $datakategori = $this->m_uom->cek('uom',$where)->result();
        ...
        $excel->getProperties()->setCreator($this->session->userdata('Nama'))
                 ...
                 ->setSubject("Satuan")
                 ->setDescription("Laporan Semua Data Satuan")
                 ->setKeywords("Data Satuan");
        $style_col = array(
                  ...
                  $excel->setActiveSheetIndex(0)->setCellValue('A1', "Data Satuan"); // Set kolom A1 dengan tulisan "DATA SISWA"
                  ...
                  $excel->setActiveSheetIndex(0)->setCellValue('C3', "Nama Satuan"); // Set kolom C3 dengan tulisan "NAMA"
                ...
                //   $excel->getActiveSheet()->getColumnDimension('E')->setWidth(30); // Set width kolom E
                  ...
                  header('Content-Disposition: attachment;filename="Satuan.xlsx"'); // Set nama file excel nya
