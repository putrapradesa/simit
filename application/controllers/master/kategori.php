...
class kategori extends CI_Controller
{
    ...
    {
        $data['Judul'] = 'Kategori';
        $data['content'] = 'master/kategori/home';

        $where = array(
            "IsDeleted" => 0
        );

        $datapt = $this->m_kategori->cek('kategori',$where)->result();
        $data['datakategori'] = $datapt;
        $data['modal_tambah_kategori'] = show_my_modal('master/kategori/modal/add', 'tambah-kategori', $data);
        $data['menu'] = 'master';
    ...
    public function tampil(){
        ...
        $data['datakategori'] = $this->m_kategori->cek('kategori',$where)->result();
        $this->load->view('master/kategori/list', $data);
    }

    

    public function add(){
        $this->form_validation->set_rules('nama_kategori[]', 'Nama Bagian', 'trim|required', 'callback_pt_check');
        ...
        // if ($this->form_validation->run() == TRUE) {
                ...
                foreach($data["nama_kategori"] as $row){
                    $where = array(
                        'Nama'      => strtoupper($row),
                        'IsDeleted' => 0,
                    );
        
                    $cek = $this->m_kategori->cek('kategori',$where)->num_rows();
                    $counter = $this->m_kategori->getdata('kategori')->num_rows();
                    $NoKategori = str_pad($counter + 1 + $index, 5, 0, STR_PAD_LEFT);
                ...
                if($kategori != ""){
                    ...
                    $out['status'] = 'form';
                    $out['msg'] = show_err_msg("Kategori ". $kategori . " Sudah Ada ", '20px');
                }
                ...
                if(!empty($out)){
                    ...
                    if(!empty($input)){
                        $result = $this->m_kategori->save_batch($input);
                        if ($result > 0) {
                            $out['status'] = 'form';
    ...
    public function update(){
        ...
        $data['datakategori'] = $this->m_kategori->cek('kategori',$where)->row();

		echo show_my_modal('master/kategori/modal/update', 'update-kategori', $data);
    }
    ...
    public function edit()
    {
        ...
        $where = array(
            ...
            $kategori = $this->m_kategori->cek('kategori',$where)->row();
            $wherecek = array(
                'Nama'      => strtoupper($data['nama_kategori'])
            );

            $cek = $this->m_kategori->cek('kategori',$wherecek)->num_rows();

            if($cek > 0){
                $out['status'] = 'form';
                $out['msg'] = show_err_msg("Kategori ". $data['nama_kategori'] . " Sudah Ada \r\n", '20px');
                echo json_encode($out);
            }else{
                $data = array(
                    'NoKategori'=> $kategori->NoKategori,
                    'Nama'      => strtoupper($data['nama_kategori']),
                    'CreateBy'  => $kategori->CreateBy,
                    'CreateUtc' => $kategori->CreateUtc,
                ...
                $result = $this->m_kategori->update('kategori',$data);

                if ($result > 0) {
                    $out['status'] = 'form';
                    $out['msg'] = show_succ_msg('Data Kategori Berhasil diupdate', '20px');
                } else {
                    $out['status'] = 'form';
                    $out['msg'] = show_succ_msg('Data Kategori Gagal diupdate', '20px');
                }
    ...
    public function delete()
    {
        ...
        $data = array(
            ...
            'CreateBy'  => $kategori->CreateBy,
            'CreateUtc' => $kategori->CreateUtc,
            'UpdateBy'  => $kategori->UpdateBy,
            'UpdateUtc' => $kategori->UpdateUtc,
            'Id'        => $kategori->Id,
            'IsDeleted' => 1
        );
    ...
    public function excel(){
        ...
        $datadivisi = $this->m_kategori->cek('kategori',$where)->result();
        ...
        $style_col = array(
                  ...
                  $excel->setActiveSheetIndex(0)->setCellValue('A1', "Data Kategori"); // Set kolom A1 dengan tulisan "DATA SISWA"
                  ...
                  $excel->setActiveSheetIndex(0)->setCellValue('C3', "Nama Kategori"); // Set kolom C3 dengan tulisan "NAMA"
                ...
                //   $excel->getActiveSheet()->getColumnDimension('D')->setWidth(100); // Set width kolom D
                //   $excel->getActiveSheet()->getColumnDimension('E')->setWidth(20); // Set width kolom E
                  ...
                  header('Content-Disposition: attachment;filename="Kategori.xlsx"'); // Set nama file excel nya
