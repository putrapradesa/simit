...
class supplier extends CI_Controller
{
    ...
    {
        $data['Judul'] = 'Supplier';
        $data['content'] = 'master/supplier/home';

        $where = array(
            "IsDeleted" => 0
        );

        $datapt = $this->m_supplier->cek('supplier',$where)->result();
        $data['datasupplier'] = $datapt;
        $data['modal_tambah_supplier'] = show_my_modal('master/supplier/modal/add', 'tambah-supplier', $data);
        $data['menu'] = 'master';
    ...
    public function tampil(){
        ...
        $data['datasupplier'] = $this->m_supplier->cek('supplier',$where)->result();
    ...
    public function add(){
        $this->form_validation->set_rules('nama_supplier[]', 'Nama Bagian', 'trim|required', 'callback_pt_check');
        $this->form_validation->set_rules('alamat_supplier[]', 'Nama Bagian', 'trim|required', 'callback_pt_check');
        $this->form_validation->set_rules('no_telp[]', 'Nama Bagian', 'trim|required', 'callback_pt_check');
        ...
        // if ($this->form_validation->run() == TRUE) {
                ...
                foreach($data["nama_supplier"] as $row){
                    $where = array(
                        'Nama'      => strtoupper($row),
                        'IsDeleted' => 0
                    );
        
                    $cek = $this->m_supplier->cek('supplier',$where)->num_rows();
                    $counter = $this->m_supplier->getdata('supplier')->num_rows();
                    $NoSupplier = str_pad($counter + 1 + $index, 5, 0, STR_PAD_LEFT);
                    ...
                    if($cek > 0 )
                    {
                        ...
                        array_push($input, array(
                            ...
                            'Nama'      => strtoupper($row),
                            'Alamat'    => strtoupper($alamat[$index]),
                            'NoTelp'    => $notelp[$index],
                ...
                if($supplier != ""){
                    ...
                    $out['status'] = 'form';
                    $out['msg'] = show_err_msg("Supplier ". $supplier . " Sudah Ada ", '20px');
                }
                ...
                if(!empty($out)){
                    ...
                    if(!empty($input)){
                        $result = $this->m_supplier->save_batch($input);
                        if ($result > 0) {
                            $out['status'] = 'form';
    ...
    public function update(){
        ...
        $data['datasupplier'] = $this->m_supplier->cek('supplier',$where)->row();

		echo show_my_modal('master/supplier/modal/update', 'update-supplier', $data);
    }
    ...
    public function edit()
    {
        ...
        $where = array(
            ...
            $supplier = $this->m_supplier->cek('supplier',$where)->row();
            $wherecek = array(
                'Nama'          => strtoupper($data['nama_supplier']),
                'IsDeleted'    => 0
            );

            $cek = $this->m_supplier->cek('supplier',$wherecek)->num_rows();

            if($cek > 0){
                $out['status'] = 'form';
                $out['msg'] = show_err_msg("Supplier ". $data['nama_supplier'] . " Sudah Ada \r\n", '20px');
                echo json_encode($out);
            }else{
                $data = array(
                    'NoSupplier'=> $supplier->NoSupplier,
                    'Nama'      => strtoupper($data['nama_supplier']),
                    'Alamat'    => strtoupper($data['alamat_supplier']),
                    'NoTelp'    => $data['no_telp'],
                    'CreatedBy'  => $supplier->CreatedBy,
                    'CreatedUtc' => $supplier->CreatedUtc,
                ...
                $result = $this->m_supplier->update('supplier',$data);
    ...
    public function excel(){
        ...
        $datadivisi = $this->m_supplier->cek('supplier',$where)->result();
        ...
        $style_col = array(
                  ...
                  $excel->setActiveSheetIndex(0)->setCellValue('A1', "Data Supplier"); // Set kolom A1 dengan tulisan "DATA SISWA"
                  ...
                  $excel->setActiveSheetIndex(0)->setCellValue('C3', "Nama Supplier"); // Set kolom C3 dengan tulisan "NAMA"
                  ...
                  header('Content-Disposition: attachment;filename="Supplier.xlsx"'); // Set nama file excel nya
